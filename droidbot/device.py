...
class Device(object):
    ...
    def grant_permission(self, app):
        """
        Grant the required permissions for the current app
        """
        if isinstance(app, str):
            package_name = app
        elif isinstance(app, App):
        ...
        r = self.adb.shell(f"dumpsys package {package_name}")
        
        permissions = re.findall(r'requested permissions:\s*(.*?)\s*install permissions:', r, re.DOTALL)
        permissions_list = permissions[0].strip().split('\n') if len(permissions) > 0 else []
        for i in range(len(permissions_list)):
            permissions_list[i] = permissions_list[i].strip()
        ...
        for permission in permissions_list:
            try:
                self.adb.shell(f"pm grant {package_name} {permission}")
                print("Grant permission:", permission)
            except:
                print("Fail grant permission:", permission)
                pass
        print("################################################")
        return None