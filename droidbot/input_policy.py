...
class UtgGreedySearchPolicy(UtgBasedInputPolicy):
    ...
    def generate_event_based_on_utg(self):
        ...
        self.calculate_activity_coverage_rate("droidbot")
    ...
    def calculate_activity_coverage_rate(self, method):
        app_activities = self.app.activities
        activity_num = len(app_activities)
        for state_str in self.utg.G.nodes():
            state = self.utg.G.nodes[state_str]["state"]
            # print(state.foreground_activity)  # com.simplemobiletools.dialer/.activities.MainActivity
            ...
            # 匹配探索到的 activity 是否在该 app 当中，然后加到 set 里面
            if activity_name_full in app_activities:
                self.explored_activities.add(activity_name_full)
            if activity_name in app_activities:
                self.explored_activities.add(activity_name)
        # print(self.explored_activities)
        activities_coverage_rate = len(self.explored_activities) / activity_num
        ...
        time_coverage_rate_path = os.path.join(self.device.output_dir, f'time_coverage_rate_{method}.csv')
